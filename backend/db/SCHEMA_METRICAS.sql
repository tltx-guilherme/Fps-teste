-- ============================================================================
-- ARQUITETURA DE MÉTRICAS ESCALÁVEL - FPS ANALYTICS
-- ============================================================================
-- Preparada para 10M+ registros com otimização incremental
-- ============================================================================
...
-- Partições por mês (auto-criadas)
CREATE TABLE transactions_raw_2026_02 PARTITION OF transactions_raw
  FOR VALUES FROM ('2026-02-01') TO ('2026-03-01');

CREATE TABLE transactions_raw_2026_03 PARTITION OF transactions_raw
...
CREATE TABLE IF NOT EXISTS metrics_hourly (
  ...
  categorias JSONB DEFAULT '{}',  -- {Boletim: 10, Financeiro: 5, ...}
  tipos_erro JSONB DEFAULT '{}',  -- {500: 3, 404: 2, ...}
  saude_geral TEXT DEFAULT 'NORMAL',  -- NORMAL | SLOW | ERROR
...
-- ============================================================================
-- 3. TABELA DE CONTROLE (para updates incrementais)
-- ============================================================================

CREATE TABLE IF NOT EXISTS metrics_control (
  id BIGSERIAL PRIMARY KEY,
  metrica_tipo TEXT NOT NULL UNIQUE,  -- 'hourly' | 'daily' | 'monthly'
...
-- ============================================================================
-- 4. FUNÇÃO PLPGSQL - UPDATE INCREMENTAL EFICIENTE
-- ============================================================================
...
-- ============================================================================
-- 5. TRIGGER AUTOMÁTICO (opcional - chamar a cada 5min é melhor)
-- ============================================================================

-- Para Supabase Free, melhor fazer via cron ou chamar do Node.js
-- CREATE OR REPLACE FUNCTION trigger_update_metrics()
-- RETURNS TRIGGER AS $$
-- BEGIN
--   -- Chama update_all_metrics() a cada 5 min
--   RETURN NEW;
-- END;
-- $$ LANGUAGE plpgsql;
